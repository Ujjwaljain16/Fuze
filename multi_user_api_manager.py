...
class MultiUserAPIManager:
    ...
    def add_user_api_key(self, user_id: int, api_key: str, api_key_name: str = None) -> bool:
        ...
        try:
            ...
            # Save to database
            self.save_user_api_key_to_db(user_api_key)
    ...
    def get_user_api_key(self, user_id: int) -> Optional[str]:
        """Get user's API key (returns None if not found)"""
        try:
            ...
            if user_id in self.user_api_keys:
                ...
                if user_api_key.status == APIKeyStatus.ACTIVE:
                    # In a real implementation, you'd decrypt the stored key
                    # For now, we'll return the default key
    ...
    def save_user_api_key_to_db(self, user_api_key: UserAPIKey):
        """Save user API key to database"""
        try:
            # In a real implementation, you'd store this in a UserAPIKeys table
