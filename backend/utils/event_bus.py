...
def read_events_for_replay(user_id: int, last_event_id: Optional[str] = None, count: int = 50) -> List[Dict[str, Any]]:
    """
    Read events for a user from Redis Stream starting from last_event_id using XREAD.
    Used by SSE Gateway to replay missed events upon reconnection.
    """
    events = []
    try:
        from utils.redis_utils import redis_cache
        ...
        logger.warning(f"read_events_for_replay_failed: {e}", extra={"user_id": user_id})
