...
_gemini_analyzers: Dict[int, GeminiAnalyzer] = {}
...
def get_cached_gemini_analyzer(user_id: int = None):
    ...
    # Use user_id as cache key (or 0 for default)
    cache_key = user_id if user_id else 0
    
    # Check if we need to create a new instance or refresh
    if (cache_key not in _gemini_analyzers or 
        cache_key not in _gemini_last_used or 
        current_time - _gemini_last_used[cache_key] > _gemini_cache_timeout):
        
        try:
            _gemini_analyzers[cache_key] = GeminiAnalyzer(api_key=api_key)
            _gemini_last_used[cache_key] = current_time
            logger.info(f"Created new cached GeminiAnalyzer instance for user {user_id or 'default'}")
        except Exception as e:
...
class UnifiedDataLayer:
    ...
    def normalize_content_data(self, content: Any, analysis: Optional[Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if analysis:
                ...
                # Also get from analysis_data JSON
                if analysis.analysis_data:
            ...
            # Determine content type (prefer analysis_data JSON)
            content_type = 'article'  # default
            if analysis:
                if analysis.analysis_data and 'content_type' in analysis.analysis_data:
                    content_type = analysis.analysis_data.get('content_type', analysis.content_type or 'article')
                else:
                    content_type = analysis.content_type or 'article'
            
            # Determine difficulty (prefer analysis_data JSON)
            difficulty = 'intermediate'  # default
            if analysis:
                if analysis.analysis_data and 'difficulty' in analysis.analysis_data:
                    difficulty = analysis.analysis_data.get('difficulty', analysis.difficulty_level or 'intermediate')
                else:
    ...
    def _get_content_from_db_with_session(self, user_id: int, request: UnifiedRecommendationRequest, session) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # OPTIMIZATION 4: Use proper ordering (no limit - use ALL user content for best recommendations)
...
class FastSemanticEngine:
    ...
    def _generate_gemini_reasoning_fast(self, content: Dict, similarity: float, tech_overlap: float, project_id: Optional[int], request: UnifiedRecommendationRequest) -> str:
        ...
        try:
            gemini_analyzer = get_cached_gemini_analyzer(user_id=request.user_id)
...
class ContextAwareEngine:
        ...
        try:
            ...
            try:
                ...
                if query_embedding is not None:
                    # Use stored embeddings from DB for fast similarity calculation
                    for content in content_list:
                        content_embedding = content.get('embedding')
                        if content_embedding is not None and len(content_embedding) > 0:
                            # Calculate cosine similarity using stored embeddings (FAST!)
            ...
            # Calculate ML (TF-IDF) scores for all content
            ml_scores = {}
            try:
    ...
    def _calculate_score_components(self, content: Dict, context: Dict, precomputed_similarity: float = None) -> Dict[str, float]:
        ...
        # Semantic similarity - use precomputed if available (FAST!)
